...
create table OFFER_DEMO(
  offerid varchar(36),
  coverage int,
  premium int, 
  inquiryid varchar(36),
  constraint fk_inquiryid foreign key(inquiryid) references INQUIRY_DEMO(inquiryid)
);


create trigger UPDATE_OFFER_ID_ON_INQUIRY_DEMO 
after insert on OFFER_DEMO
for each row
	UPDATE INQUIRY_DEMO
